...
const HandOffModal = ({ sourcePlayer, sourceUnitType, item, players, onConfirm, onClose }) => {
  ...
  const labelStyle = { color: '#8b7355', fontSize: '0.68rem', fontWeight: '800', letterSpacing: '0.1em', textTransform: 'uppercase', display: 'block', marginBottom: '0.35rem' };
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'linear-gradient(145deg, #1a0f0a, #0f0805)', border: `3px solid ${gold}`, borderRadius: '12px', padding: '1.5rem', width: '460px', maxWidth: '95%', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.95)' }}>

        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: '1.25rem' }}>
          <div style={{ fontSize: '1.75rem', marginBottom: '0.25rem' }}>🤝</div>
          <div style={{ color: gold, fontWeight: '900', fontSize: '1rem', fontFamily: '"Cinzel", Georgia, serif' }}>HAND OFF ITEM</div>
          <div style={{ color: '#6b7280', fontSize: '0.72rem', marginTop: '0.2rem' }}>
            Giving: <span style={{ color: '#fbbf24', fontWeight: '800' }}>{item.name}</span>
            {' '}from <span style={{ color: '#9ca3af' }}>{sourcePlayer.playerName}</span>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <select
            ...
            style={{ width: '100%', background: '#0a0503', color: gold, padding: '0.6rem 0.75rem', borderRadius: '6px', border: '1px solid #5a4a3a', fontFamily: 'inherit', fontSize: '0.875rem' }}
          >
              ...
              {getAllUnits(targetPlayer).map(u => {
                ...
                return (
                  <div key={u.unitType} onClick={() => !u.isDead && !isSelf && handleSelectUnit(u.unitType)} style={{
                    ...
                    border: `2px solid ${selected ? gold : isFull ? 'rgba(249,115,22,0.3)' : 'rgba(90,74,58,0.3)'}`,
                    borderRadius: '7px', cursor: (u.isDead || isSelf) ? 'not-allowed' : 'pointer',
                    opacity: (u.isDead || isSelf) ? 0.4 : 1,
                  }}>
                    <div style={{ flex: 1 }}>
                      <span style={{ color: selected ? gold : '#9ca3af', fontWeight: '800', fontSize: '0.82rem' }}>{u.label}</span>
                      {isSelf && <span style={{ color: '#4b5563', fontSize: '0.6rem', fontWeight: '700', marginLeft: '0.4rem' }}>SOURCE</span>}
                    </div>
                    <div style={{ width: '50px', height: '4px', background: 'rgba(0,0,0,0.5)', borderRadius: '2px', overflow: 'hidden' }}>
                      <div style={{ width: `${u.maxHp > 0 ? (u.hp / u.maxHp) * 100 : 0}%`, height: '100%', background: u.isDead ? '#374151' : '#22c55e' }} />
                    </div>
                    <span style={{ color: '#6b7280', fontSize: '0.62rem', minWidth: '32px' }}>{u.hp}/{u.maxHp}</span>
          ...
          <div style={{ marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
              <button onClick={() => { setMode('give'); setTradeItem(null); }} style={{
                padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.82rem', cursor: 'pointer',
              ...
              <button
                ...
                style={{
                  padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.82rem',
                  ...
                  color: mode === 'trade' ? '#e9d5ff' : canTrade ? '#9ca3af' : '#374151',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
              {targetUnitItems.map(it => (
                <div key={it.id} onClick={() => setTradeItem(it)} style={{
                  ...
                  <span style={{ color: tradeItem?.id === it.id ? '#e9d5ff' : '#9ca3af', fontWeight: '800', fontSize: '0.82rem' }}>{it.name}</span>
                  <span style={{ color: '#6b7280', fontSize: '0.65rem', marginLeft: '0.5rem' }}>{it.tier}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <button disabled={!canConfirm} onClick={handleConfirm} style={{
            flex: 1, padding: '0.75rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.9rem', cursor: canConfirm ? 'pointer' : 'not-allowed',
            background: canConfirm ? 'linear-gradient(135deg, #059669, #047857)' : '#1a0f0a',
            border: `2px solid ${canConfirm ? '#10b981' : '#374151'}`,
            color: canConfirm ? '#d1fae5' : '#4a3322',
          }}>✓ Confirm</button>
          <button onClick={onClose} style={{ flex: 1, padding: '0.75rem', background: 'rgba(127,29,29,0.3)', border: '2px solid #7f1d1d', color: '#fca5a5', borderRadius: '8px', cursor: 'pointer', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.9rem' }}>✕ Cancel</button>
        </div>
      </div>
    </div>
  );
