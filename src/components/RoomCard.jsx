...
const STATUS_CONFIG = {
  Idle:   { color: '#6b7280', bg: 'rgba(107,114,128,0.12)', border: 'rgba(107,114,128,0.35)', dot: '#4b5563',  label: '⬜ Idle'    },
...
const modal = { background: 'linear-gradient(145deg,#1a0f0a,#0f0805)', border: `3px solid ${gold}`, borderRadius: '12px', padding: '1.5rem', width: '95%', maxWidth: '420px', maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.95)' };
const modalTitle = { color: gold, fontFamily: '"Cinzel",Georgia,serif', fontWeight: '900', fontSize: '1rem', textAlign: 'center', marginBottom: '1rem', letterSpacing: '0.08em' };
  ...
  return (
    <div onClick={onClose} style={overlay}>
      <div onClick={e => e.stopPropagation()} style={modal}>
        ...
        <div style={{ color: '#6b7280', fontSize: '0.72rem', textAlign: 'center', marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem', maxHeight: '280px', overflowY: 'auto', marginBottom: '1rem' }}>
          {allUnits.map(u => {
            ...
            return (
              <div key={`${u.playerId}-${u.unitType}`} onClick={() => !isLockedOut && toggle(u.playerId, u.unitType)} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.5rem 0.75rem', borderRadius: '7px', cursor: isLockedOut ? 'not-allowed' : 'pointer', opacity: isLockedOut ? 0.4 : 1, background: sel ? 'rgba(251,191,36,0.1)' : 'rgba(0,0,0,0.3)', border: `1px solid ${sel ? 'rgba(251,191,36,0.45)' : isLockedOut ? 'rgba(192,132,252,0.3)' : 'rgba(90,74,58,0.3)'}` }}>
                <div style={{ width: '14px', height: '14px', borderRadius: '3px', border: `2px solid ${sel ? '#fbbf24' : '#4b5563'}`, background: sel ? '#fbbf24' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.55rem', color: '#000', fontWeight: '900', flexShrink: 0 }}>{sel && '✓'}</div>
                <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: u.playerColor || gold, flexShrink: 0 }} />
                <span style={{ color: sel ? '#fbbf24' : '#9ca3af', fontWeight: '800', fontSize: '0.82rem', flex: 1 }}>{u.playerName} · {u.label}</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
          <button disabled={!meetsMin} onClick={() => meetsMin && onConfirm(selected)} style={{ padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.85rem', cursor: meetsMin ? 'pointer' : 'not-allowed', background: meetsMin ? 'linear-gradient(135deg,#059669,#047857)' : '#0a0503', border: `2px solid ${meetsMin ? '#10b981' : '#374151'}`, color: meetsMin ? '#d1fae5' : '#374151' }}>⚡ Activate</button>
          <button onClick={onClose} style={{ padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.85rem', cursor: 'pointer', background: 'rgba(127,29,29,0.3)', border: '2px solid #7f1d1d', color: '#fca5a5' }}>✕ Cancel</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div onClick={onClose} style={overlay}>
      <div onClick={e => e.stopPropagation()} style={modal}>
        ...
        <div style={{ color: '#6b7280', fontSize: '0.65rem', fontWeight: '700', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.35rem' }}>Select Betrayer(s)</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', maxHeight: '260px', overflowY: 'auto', marginBottom: '1rem' }}>
          {allUnits.map(u => {
            ...
            return (
              <div key={`${u.playerId}-${u.unitType}`} onClick={() => toggle(u.playerId, u.unitType, u.playerName, u.label)} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.45rem 0.7rem', borderRadius: '7px', cursor: 'pointer', background: sel ? 'rgba(239,68,68,0.1)' : 'rgba(0,0,0,0.3)', border: `1px solid ${sel ? 'rgba(239,68,68,0.45)' : 'rgba(90,74,58,0.3)'}` }}>
                <div style={{ width: '14px', height: '14px', borderRadius: '3px', border: `2px solid ${sel ? '#ef4444' : '#4b5563'}`, background: sel ? '#ef4444' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.55rem', color: '#fff', fontWeight: '900', flexShrink: 0 }}>{sel && '🐍'}</div>
                <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: u.playerColor || gold, flexShrink: 0 }} />
                <span style={{ color: sel ? '#fca5a5' : '#9ca3af', fontWeight: '800', fontSize: '0.82rem', flex: 1 }}>{u.playerName} · {u.label}</span>
              </div>
            );
          })}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
          <button onClick={() => onConfirm(betrayers)} style={{ padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.85rem', cursor: 'pointer', background: 'linear-gradient(135deg,#059669,#047857)', border: '2px solid #10b981', color: '#d1fae5' }}>✓ Confirm</button>
          <button onClick={onClose} style={{ padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.85rem', cursor: 'pointer', background: 'rgba(127,29,29,0.3)', border: '2px solid #7f1d1d', color: '#fca5a5' }}>✕ Cancel</button>
        </div>
      </div>
    </div>
  ...
  return (
    <div onClick={onClose} style={overlay}>
      <div onClick={e => e.stopPropagation()} style={modal}>
        ...
        <div style={{ color: '#6b7280', fontSize: '0.65rem', fontWeight: '700', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.35rem' }}>Toggle Lock Status</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', maxHeight: '260px', overflowY: 'auto', marginBottom: '1rem' }}>
          {allUnits.map(u => {
            const sel = isLocked(u.playerId, u.unitType);
            return (
              <div key={`${u.playerId}-${u.unitType}`} onClick={() => toggle(u.playerId, u.unitType, u.playerName, u.label)} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.45rem 0.7rem', borderRadius: '7px', cursor: 'pointer', background: sel ? 'rgba(192,132,252,0.1)' : 'rgba(0,0,0,0.3)', border: `1px solid ${sel ? 'rgba(192,132,252,0.45)' : 'rgba(90,74,58,0.3)'}` }}>
                <div style={{ width: '14px', height: '14px', borderRadius: '3px', border: `2px solid ${sel ? '#a855f7' : '#4b5563'}`, background: sel ? '#a855f7' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6rem', flexShrink: 0 }}>{sel && '🔒'}</div>
                <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: u.playerColor || gold, flexShrink: 0 }} />
                <span style={{ color: sel ? '#c084fc' : '#9ca3af', fontWeight: '800', fontSize: '0.82rem', flex: 1 }}>{u.playerName} · {u.label}</span>
              </div>
            );
          })}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
          <button onClick={() => onConfirm(locked)} style={{ padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.85rem', cursor: 'pointer', background: 'linear-gradient(135deg,#059669,#047857)', border: '2px solid #10b981', color: '#d1fae5' }}>✓ Confirm</button>
          <button onClick={onClose} style={{ padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.85rem', cursor: 'pointer', background: 'rgba(127,29,29,0.3)', border: '2px solid #7f1d1d', color: '#fca5a5' }}>✕ Cancel</button>
        </div>
      </div>
    </div>
  ...
  return (
    <div onClick={onClose} style={overlay}>
      <div onClick={e => e.stopPropagation()} style={{ ...modal, maxWidth: '400px' }}>
        ...
        {!rolled ? (
          <button onClick={rollLoot} style={{ width: '100%', padding: '0.85rem', background: 'linear-gradient(135deg,#92400e,#78350f)', border: '2px solid #eab308', color: '#fde68a', borderRadius: '8px', cursor: 'pointer', fontFamily: 'inherit', fontWeight: '800', fontSize: '1rem', letterSpacing: '0.05em', marginBottom: '0.75rem' }}>
            🎲 Roll Loot
          </button>
        ) : (
          <div style={{ marginBottom: '1rem' }}>
            <div style={{ color: gold, fontWeight: '800', fontSize: '0.8rem', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.5rem', textAlign: 'center' }}>✨ Loot Dropped</div>
            {rolled.length === 0 ? (
              <div style={{ textAlign: 'center', color: '#4b5563', fontSize: '0.85rem', padding: '1rem' }}>No items dropped.</div>
            ) : (
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.6rem 0.85rem', background: tc.bg, border: `2px solid ${tc.border}`, borderRadius: '8px' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {item.description && <div style={{ color: '#6b7280', fontSize: '0.68rem' }}>{item.description}</div>}
                      </div>
            ...
            <button onClick={() => setRolled(null)} style={{ width: '100%', marginTop: '0.5rem', padding: '0.45rem', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(90,74,58,0.3)', borderRadius: '6px', color: '#6b7280', fontFamily: 'inherit', fontWeight: '700', fontSize: '0.72rem', cursor: 'pointer' }}>🔄 Reroll</button>
          </div>
        )}

        <button onClick={onClose} style={{ width: '100%', padding: '0.6rem', background: 'rgba(127,29,29,0.2)', border: '1px solid #7f1d1d', borderRadius: '8px', color: '#fca5a5', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.85rem', cursor: 'pointer' }}>✕ Close</button>
      </div>
    </div>
  );
};
    ...
    <div onClick={onClose} style={overlay}>
      <div onClick={e => e.stopPropagation()} style={{ ...modal, maxWidth: '460px' }}>
        ...
        {items.map((item, i) => {
          ...
          return (
            <div key={i} style={{ marginBottom: '0.75rem' }}>
              ...
              {/* Unit picker */}
              {!assigned && (
                <div style={{ background: 'rgba(0,0,0,0.4)', border: `2px solid ${gold}`, borderTop: 'none', borderRadius: '0 0 8px 8px', padding: '0.5rem', maxHeight: '160px', overflowY: 'auto' }}>
                  {allUnits.map(u => (
                    <div key={`${u.playerId}-${u.unitType}`} onClick={() => setAssignments(prev => { const next = [...prev]; next[i] = u; return next; })} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.35rem 0.5rem', borderRadius: '5px', cursor: 'pointer', marginBottom: '0.15rem', background: 'transparent' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(201,169,97,0.08)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                      <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: u.playerColor || gold, flexShrink: 0 }} />
                      <span style={{ color: '#9ca3af', fontSize: '0.78rem', fontWeight: '700', flex: 1 }}>{u.playerName} · {u.label}</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginTop: '0.5rem' }}>
          <button disabled={!allAssigned} onClick={allAssigned ? handleConfirm : undefined} style={{ padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.85rem', cursor: allAssigned ? 'pointer' : 'not-allowed', background: allAssigned ? 'linear-gradient(135deg,#059669,#047857)' : '#0a0503', border: `2px solid ${allAssigned ? '#10b981' : '#374151'}`, color: allAssigned ? '#d1fae5' : '#374151' }}>✓ Give Loot</button>
          <button onClick={onClose} style={{ padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.85rem', cursor: 'pointer', background: 'rgba(127,29,29,0.3)', border: '2px solid #7f1d1d', color: '#fca5a5' }}>✕ Skip</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div onClick={onClose} style={overlay}>
      <div onClick={e => e.stopPropagation()} style={{ ...modal, maxWidth: '380px' }}>
        ...
        {locked.length === 0 ? (
          <div style={{ textAlign: 'center', color: '#4b5563', fontSize: '0.85rem', padding: '1rem' }}>No one is locked out.</div>
        ) : (
          <>
            <div style={{ color: '#6b7280', fontSize: '0.72rem', textAlign: 'center', marginBottom: '0.75rem' }}>Select units to unlock</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem', marginBottom: '1rem' }}>
              {locked.map(l => {
                const key = `${l.playerId}-${l.unitType}`;
                const sel = toUnlock.includes(key);
                return (
                  <div key={key} onClick={() => toggle(l.playerId, l.unitType)} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.5rem 0.75rem', borderRadius: '7px', cursor: 'pointer', background: sel ? 'rgba(74,222,128,0.1)' : 'rgba(0,0,0,0.3)', border: `1px solid ${sel ? 'rgba(74,222,128,0.45)' : 'rgba(192,132,252,0.35)'}` }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
          <button disabled={toUnlock.length === 0} onClick={toUnlock.length > 0 ? handleConfirm : undefined} style={{ padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.85rem', cursor: toUnlock.length > 0 ? 'pointer' : 'not-allowed', background: toUnlock.length > 0 ? 'linear-gradient(135deg,#059669,#047857)' : '#0a0503', border: `2px solid ${toUnlock.length > 0 ? '#10b981' : '#374151'}`, color: toUnlock.length > 0 ? '#d1fae5' : '#374151' }}>🔓 Unlock</button>
          <button onClick={onClose} style={{ padding: '0.65rem', borderRadius: '8px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.85rem', cursor: 'pointer', background: 'rgba(127,29,29,0.3)', border: '2px solid #7f1d1d', color: '#fca5a5' }}>✕ Cancel</button>
        </div>
      </div>
    </div>
    ...
    <div style={{ background: 'linear-gradient(135deg,rgba(17,24,39,0.95),rgba(31,41,55,0.85))', borderRadius: '12px', padding: '1rem', fontFamily: '"Rajdhani","Cinzel",sans-serif', boxShadow: room.status === 'Active' ? '0 0 24px rgba(251,191,36,0.2),0 8px 32px rgba(0,0,0,0.6)' : '0 8px 32px rgba(0,0,0,0.6),inset 0 1px 0 rgba(255,255,255,0.05)', border: `2px solid ${sc.border}`, opacity: room.archived ? 0.5 : 1, transition: 'all 0.3s' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem', paddingBottom: '0.75rem', borderBottom: '1px solid rgba(201,169,97,0.2)' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ color: gold, fontSize: '1.1rem', fontWeight: '800', letterSpacing: '0.05em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{room.name || 'Unnamed Room'}</div>
          <div style={{ color: '#6b7280', fontSize: '0.7rem', letterSpacing: '0.08em', marginTop: '0.1rem' }}>
            {sc.label}
        ...
        <div style={{ display: 'flex', gap: '0.3rem', flexShrink: 0 }}>
          <button onClick={onEdit}    style={{ padding: '0.25rem 0.55rem', background: 'rgba(201,169,97,0.1)',  border: '1px solid rgba(201,169,97,0.35)', borderRadius: '5px', color: gold,     cursor: 'pointer', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.68rem' }}>✏️</button>
          <button onClick={onArchive} style={{ padding: '0.25rem 0.55rem', background: 'rgba(0,0,0,0.25)',      border: '1px solid rgba(90,74,58,0.3)',   borderRadius: '5px', color: '#6b7280', cursor: 'pointer', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.68rem' }}>📁</button>
          <button onClick={onDelete}  style={{ padding: '0.25rem 0.55rem', background: 'rgba(127,29,29,0.15)', border: '1px solid rgba(127,29,29,0.4)',  borderRadius: '5px', color: '#fca5a5',cursor: 'pointer', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.68rem' }}>🗑</button>
        </div>
      </div>

      {/* ── Description ── */}
      {room.description && (
        <div style={{ color: '#8b7355', fontSize: '0.78rem', lineHeight: 1.5, marginBottom: '0.75rem', padding: '0.5rem 0.75rem', background: 'rgba(0,0,0,0.25)', borderRadius: '6px', borderLeft: `3px solid ${sc.border}` }}>{room.description}</div>
      )}
      ...
      {room.notes && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.3rem 0.65rem', background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(90,74,58,0.25)', borderRadius: '6px', marginBottom: '0.5rem' }}>
          ...
          <span style={{ color: '#6b7280', fontSize: '0.7rem', fontStyle: 'italic', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{room.notes}</span>
        </div>
      )}
      ...
      {room.hiddenCostEnabled && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.45rem 0.75rem', background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: '6px', marginBottom: '0.65rem' }}>
          ...
          {!room.hiddenCostRevealed && <button onClick={() => onUpdate({ hiddenCostRevealed: true })} style={{ padding: '0.18rem 0.55rem', borderRadius: '5px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.62rem', cursor: 'pointer', background: 'rgba(251,191,36,0.1)', border: '1px solid rgba(251,191,36,0.35)', color: '#fbbf24' }}>Reveal HP Cost</button>}
        </div>
      )}
      ...
      {(room.charactersPresent || []).length > 0 && (
        <div style={{ marginBottom: '0.65rem' }}>
          <div style={{ color: '#6b7280', fontSize: '0.62rem', fontWeight: '700', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.3rem' }}>Inside Room</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.25rem' }}>
            {room.charactersPresent.map(ch => {
              const p = players.find(pl => pl.id === ch.playerId);
              if (!p) return null;
              return <div key={`${ch.playerId}-${ch.unitType}`} style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', padding: '0.15rem 0.5rem', background: 'rgba(0,0,0,0.35)', border: `1px solid ${p.playerColor || gold}35`, borderRadius: '20px' }}><div style={{ width: '5px', height: '5px', borderRadius: '50%', background: p.playerColor || gold }} /><span style={{ color: '#9ca3af', fontSize: '0.62rem', fontWeight: '700' }}>{p.playerName} · {unitLabel(p, ch.unitType)}</span></div>;
            })}
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '0.25rem', marginBottom: '0.5rem' }}>
        {Object.entries(STATUS_CONFIG).map(([s, cfg]) => (
          <button key={s} onClick={() => onSetStatus(s)} style={{ padding: '0.3rem 0.2rem', borderRadius: '5px', fontFamily: 'inherit', fontWeight: '800', fontSize: '0.55rem', cursor: 'pointer', textAlign: 'center', background: room.status === s ? cfg.bg : 'rgba(0,0,0,0.3)', border: `1px solid ${room.status === s ? cfg.border : 'rgba(55,65,81,0.4)'}`, color: room.status === s ? cfg.color : '#4b5563' }}>{cfg.label}</button>
        ))}
      </div>

      {/* ── Action buttons ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.35rem', marginBottom: '0.35rem' }}>
        <button onClick={() => setShowActivate(true)} style={{ padding: '0.5rem', borderRadius: '7px', fontFamily: 'inherit', fontWeight: '900', fontSize: '0.72rem', cursor: 'pointer', background: 'rgba(251,191,36,0.1)', border: '1px solid rgba(251,191,36,0.4)', color: '#fbbf24' }}>⚡ Activate</button>
        {room.lootMode && room.lootMode !== 'none' && <button onClick={() => { const rolled = rollRoomLoot(); setLootRollResult(rolled); setPresentSnapshot(room.charactersPresent || []); setShowLootDisburse(true); }} style={{ padding: '0.5rem', borderRadius: '7px', fontFamily: 'inherit', fontWeight: '900', fontSize: '0.72rem', cursor: 'pointer', background: 'rgba(201,169,97,0.1)', border: '1px solid rgba(201,169,97,0.4)', color: gold }}>🎲 Roll Loot</button>}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: room.betrayalEnabled ? '1fr 1fr 1fr 1fr' : '1fr 1fr 1fr', gap: '0.35rem' }}>
        <button onClick={handlePass} style={{ padding: '0.45rem', borderRadius: '7px', fontFamily: 'inherit', fontWeight: '900', fontSize: '0.72rem', cursor: 'pointer', background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.4)', color: '#4ade80' }}>✓ Pass</button>
        <button onClick={handleFail} style={{ padding: '0.45rem', borderRadius: '7px', fontFamily: 'inherit', fontWeight: '900', fontSize: '0.72rem', cursor: 'pointer', background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.4)', color: '#f87171' }}>✗ Fail</button>
        <button onClick={handleLockedOut} style={{ padding: '0.45rem', borderRadius: '7px', fontFamily: 'inherit', fontWeight: '900', fontSize: '0.72rem', cursor: 'pointer', background: 'rgba(192,132,252,0.1)', border: '1px solid rgba(192,132,252,0.4)', color: '#c084fc' }}>🔒 Locked Out</button>
        {room.betrayalEnabled && <button onClick={() => setShowBetrayal(true)} style={{ padding: '0.45rem', borderRadius: '7px', fontFamily: 'inherit', fontWeight: '900', fontSize: '0.72rem', cursor: 'pointer', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.4)', color: '#f87171' }}>🐍 Betrayal</button>}
      </div>
